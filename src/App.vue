...
export default {
  ...
  mounted() {
    window.addEventListener("resize", this.calculateMaxGraphEements);
  },

  beforeUnmount() {
    window.removeEventListener("resize", this.calculateMaxGraphEements);
  },
  ...
  watch: {
    selectedTicker() {
      this.graph = [];
      this.$nextTick().then(this.calculateMaxGraphEements);
      // this.$nextTick().then(this.graphElementWidth = this.$refs.graphElement[0].clientWidth);
    },
